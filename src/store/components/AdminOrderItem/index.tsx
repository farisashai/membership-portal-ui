    ...
    <Row align="middle" justify="center" className="order-item">
      ...
      >
        <form>
          <Form.Item className="note">
            <Input.TextArea name="note" value={orderItem.scratchNote} onChange={(event) => setScratchNote(orderItem, event.target.value)} rows={4} />
          </Form.Item>
        </form>
      </Modal>
      <Col span={2}>
      ...
      <Col span={2}>
        <div className="item-notes">
          <Icon onClick={() => setNoteVisible(true)} component={orderItem.scratchNote ? CommentBox : CommentBoxEmpty} />
        </div>
      </Col>
    </Row>
  );
