...
import { Order, OrderItem, PatchOrderItemPayload } from '../../types';
import { getAllOrders, patchOrder } from '../storeActions';
import adminOrdersReducer from '../adminReducer';
...
const AdminOrderPageContainer: React.FC = () => {
  ...
  const refinedApiOrders: Order[] = orderList.map((element) => {
    ...
    const groupedOrderItems = orderItemsWithoutFirst.reduce((acc, curr) => {
      ...
      const existingItemIndex = newAcc.findIndex((itemElement) => itemElement.item.uuid === curr.item.uuid);
    ...
    if (value === '') {
      ...
      setSearchedOrders(
        refinedApiOrders.filter((element) => {
          const nameOfOrderUser = `${element.userInfo!.firstName} ${element.userInfo!.lastName}`.toLowerCase();
